...
class ChatQueue:
    def __init__(self):
        ...
        self.album_videos = []  # قائمة لتجميع الفيديوهات المعالجة
...
async def process_video(chat_id, message):
    ...
    تعيد الدالة قاموسًا يحتوي على بيانات الفيديو المعالج.
    """
    temp_file = None
    thumb = None
    result = None
...
async def send_album(chat_id, album_videos):
    ...
    يتم تعيين "حصريات🌈" كتعليق على أول فيديو في الألبوم.
    بعد الإرسال، يتم حذف رسائل المستخدم والتأكيد والملفات المؤقتة.
    يُرسل الألبوم إلى القناة (CHANNEL_ID) إن كانت متوفرة.
    """
    media_list = []
    ...
    # حذف رسائل المستخدم والتأكيد والملفات المؤقتة بعد الإرسال
    ...
    logging.info("انتظار 10 ثوانٍ قبل إرسال ألبوم جديد ...")
...
async def process_queue(chat_id):
    ...
    عند وصول عدد الفيديوهات إلى 10 أو انتهاء الطابور، يتم إرسال الألبوم.
    """
    ...
    try:
        while not cq.queue.empty():
            ...
            if len(cq.album_videos) >= 10:
                ...
                cq.album_videos = []
        # في حال انتهاء الطابور ووجود فيديوهات أقل من 10، نرسلها أيضًا
        if cq.album_videos:
            logging.info(f"تم تجميع {len(cq.album_videos)} فيديوهات (أقل من 10)، بدء إرسال الألبوم...")
            await send_album(chat_id, cq.album_videos)
            cq.album_videos = []
    except Exception as e:
...
async def queue_manager():
    """مدير الطوابير الأساسي لمراقبة الرسائل الواردة"""
    while True:
...
async def on_video_receive(client, message):
    ...
    - يُرسل رسالة تأكيد تخبر المستخدم بأنه تمت الإضافة.
    - تُخزن رسالة التأكيد لمزيد من الحذف بعد الإرسال.
    """
